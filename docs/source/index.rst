.. MerAnO documentation master file, created by
   sphinx-quickstart on Tue May 26 23:27:01 2020.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to MerAnO's documentation!
==================================


MerAnO is a python3 package created as part of the Bioinformatics master's degree in Bordeaux. MerAnO for "Merge & Analyze Organism" has two separate objectives.
 
 * Merge files in SBML format while modifying the identifiers. Thus, each "species" or "reaction" present in the final file is associated with an organism.
 * Analyze the modules present in a file in annotation format. The data required in KEGG allows the development of graphs illustrating the proportion of modules in each organization and compares the modules of the organizations included in the analysis.

This project is licensed under the GNU General Public License - see the `LICENSE.md <https://github.com/marlenebrs/MerAnO/blob/master/LICENSE>`__ file for details.



Documentation of MerAnO
=======================

.. toctree::
    :maxdepth: 2
    

    install
    tutorial
    api



Authors
=======

`Marie Ancelle <https://github.com/Marie-Ancelle/>`__ , `Marlène Barus <https://github.com/marlenebrs/>`__, `Mathilde Borg <https://github.com/mathildeborg/>`__ `Céline Mathez <https://github.com/cmathez/>`__ `Marieke Paardekooper <https://github.com/MariekeLP>`__ Université de Bordeaux, Bordeaux, France.

Acknowledgement
===============


**Clémence frioux** (Inria, Bordeaux) for having proposed this project to us as part of our master. And for his support which was formativeClémence frioux for proposing this project to us for . And for his support which was formative


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`


